defmodule FloUI.Scrollable.ScrollBar do
  ...
  def handle_input({:cursor_button, {_button, 0, _, _}}, :scroll_button_1, %{assigns: %{state: state}} = scene) do
    state =
  ...
  def handle_input({:cursor_button, {_button, 0, _, _}}, :scroll_button_2, %{assigns: %{state: state}} = scene) do
    state =
      Map.update!(state, :scroll_buttons, fn scroll_buttons ->
  ...
  def handle_input({:cursor_button, {_button, 1, _, _}}, :scroll_button_1, %{assigns: %{state: state}} = scene) do
    state =
      Map.update!(state, :scroll_buttons, fn scroll_buttons ->
  ...
  def handle_input({:cursor_button, {_button, 1, _, _}}, :scroll_button_2, %{assigns: %{state: state}} = scene) do
    state =
      Map.update!(state, :scroll_buttons, fn scroll_buttons ->
  ...
  def handle_input({:cursor_button, {_button, 1, _, _position}}, :scroll_bar_slider_background, %{assigns: %{state: state}} = scene) do
    state =
  ...
  def handle_input({:cursor_button, {_button, 1, _, _}}, _, %{assigns: %{state: %{direction: direction} = state}} = scene) do
    state =
      state
      |> Map.put(:scroll_bar_slider_background, :released)
      |> Map.update!(state, :scroll_buttons, fn scroll_buttons ->
  ...
  def handle_input({:cursor_button, {_button, 0, _, _}}, _, %{assigns: %{state: %{direction: direction} = state}} = scene) do
    state =
      state
      |> Map.put(:scroll_bar_slider_background, :released)
      |> Map.update!(state, :scroll_buttons, fn scroll_buttons ->
  ...
  def handle_input({:cursor_pos, position}, _, %{assigns: %{state: %{direction: direction} = state}} = scene) do
    scroll_position =
      Direction.from_vector_2(position, direction)
      |> Direction.map_horizontal(fn pos -> pos - button_width(state) / 2 end)
      |> Direction.map_vertical(fn pos -> pos - button_height(state) / 2 end)
  ...
  def handle_cast({:update_cursor_scroll, {{_, offset_y}, _}},
  ...
  def handle_cast({:update_cursor_scroll, {{offset_x, _}, _}},
  ...
  defp init_scroll_buttons(%{graph: graph, direction: direction} = state) do
    ...
    |> FloUI.Icon.add_to_graph({:flo_ui, "icons/arrow_drop_down_white.png"}, translate: {size * -1.05, size * -1.05 - 2}, pin: {48 / 2, 48 / 2}, rotate: :math.pi())
    |> rrect(
      {size, size, radius},
      id: :scroll_button_1,
      translate: {0, -2},
    ...
    |> FloUI.Icon.add_to_graph({:flo_ui, "icons/arrow_drop_down_white.png"}, translate: Vector2.add(button_2_position, {size * -1.05, size * -1.05 + 2}))
    |> rrect(
      {size, size, radius},
      id: :scroll_button_2,
      translate: {button_2_x, button_2_y + 2},
  ...
  defp send_position_change_event(%{last_scroll_position: last, scroll_position: current} = state, scene) do
