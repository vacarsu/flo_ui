defmodule FloUI.Scrollable.Components do
  alias Scenic.Graph
  alias Scenic.Primitive
  alias Scenic.Primitive.SceneRef

  alias FloUI.Scrollable
  ...
  def scrollable(%Scenic.Primitive{module: Scenic.Primitive.Component} = p, settings, builder, options) do
  ...
  def scroll_bars(%Scenic.Primitive{module: Scenic.Primitive.Component} = p, settings, options) do
  ...
  defp add_to_graph(%Graph{} = graph, module, data, options) do
    {:ok, data} = module.validate(data)
    module.add_to_graph(graph, data, options)
  end

  @spec modify(Primitive.t(), module, term, keyword) :: Primitive.t()
  defp modify(%Scenic.Primitive{module: Scenic.Primitive.Component, data: {mod, _, id}} = p, module, data, options) do
    {:ok, data} = module.validate(data)
